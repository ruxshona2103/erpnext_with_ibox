...
def get_columns():
	return [
		...
		{"label": "Ҳужжат №", "fieldname": "voucher_no", "fieldtype": "Dynamic Link",
		 "options": "voucher_type", "width": 120},
		{"label": "Маҳсулот номи", "fieldname": "item_name", "fieldtype": "Data", "width": 200},
		{"label": "Миқдори", "fieldname": "qty", "fieldtype": "Float", "precision": 2,
		 "width": 80},
		{"label": "Нархи", "fieldname": "rate", "fieldtype": "Currency", "width": 100},
		{"label": "Валюта", "fieldname": "currency", "fieldtype": "Link", "options": "Currency",
		 "width": 80},
		{"label": "Кредит", "fieldname": "credit", "fieldtype": "Currency", "width": 120},
		{"label": "Дебет", "fieldname": "debit", "fieldtype": "Currency", "width": 120},
		{"label": "Қолдиқ", "fieldname": "balance", "fieldtype": "Float", "precision": 2,
		 "width": 120},
	]
...
def get_data(filters):
	...
	# Salary Slip'lardan (faqat Employee uchun, HRMS o'rnatilgan bo'lsa)
	opening_salary = 0
	salary_slip_available = party_type == "Employee" and frappe.db.table_exists("Salary Slip")
	if salary_slip_available:
	...
	# Salary Slip'larni olish (Employee uchun, HRMS o'rnatilgan bo'lsa)
	salary_slips = []
	if salary_slip_available:
	...
	if len(data) > 1:  # Agar faqat opening balance dan ko'proq qator bo'lsa
		...
		total_credit = sum(flt(row.get('credit', 0)) for row in data if
						   row.get('voucher_type') != 'Boshlang\'ich qoldiq')
		total_debit = sum(flt(row.get('debit', 0)) for row in data if
						  row.get('voucher_type') != 'Boshlang\'ich qoldiq')
...
def _render_pdf_html(data, filters, company, summary):
	template_path = os.path.join(os.path.dirname(__file__), "akt_sverka.html")

	if not os.path.exists(template_path):
		frappe.throw(
			f"PDF шаблон топилмади: {template_path}\n"
			"akt_sverka.html файлини report папкасига қўйинг."
		)
